	...
	this.init = function(containerEl, viewNames, id, reconstruct){
		...
		if(id){
			if(!reconstruct){
				...
				var savedState = localStorage.getItem('DIRIGIBLE.application.car-service-bookings.'+ id);
				if(savedState !== null) {
		...
		if(id){
			this.layout.on('stateChanged', function(){
				...
				localStorage.setItem('DIRIGIBLE.application.car-service-bookings.'+ id, state );
			}.bind(this));			
		}
